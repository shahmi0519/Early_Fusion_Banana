...
# --- Google Drive sensor model info ---
SENSOR_MODEL_FILE = "banana_900_sensor_model_September.pth"
SENSOR_MODEL_DRIVE_ID = "1d6THWHfx_O0cMBP9tqx6e2UPW8NysE4W"

def download_sensor_model():
    if not os.path.exists(SENSOR_MODEL_FILE):
        st.warning(f"Downloading {SENSOR_MODEL_FILE} from Google Drive...")
        url = f"https://drive.google.com/uc?id={SENSOR_MODEL_DRIVE_ID}"
        gdown.download(url, SENSOR_MODEL_FILE, quiet=False)

# Download the sensor model before loading
download_sensor_model()
